...
//    @State var dataNoti: [DataNotification]
    @ObservedObject var NotificationOBS = LoadNotification()
    @ObservedObject var DetailOfNotiOBS = LoadDetailPost()
    var body: some View {
        ScrollView(showsIndicators: false){
            VStack(alignment: .leading) {
                Text("Notification").bold().font(.system(size: 20))
                ForEach(NotificationOBS.dataNotification) { item in
//                    NavigationLink(destination: DetailPost(dataDetail: DetailOfNotiOBS.loadDetailImage)) {
//
//                    }
                    HStack{
                        WebImage(url: URL(string: item.avata))
                            .resizable()
                    ...
                    .onAppear{
                        DetailOfNotiOBS.loadDataDetail(id: item.id)
                    }
                }
...
struct DataNotification: Identifiable, Hashable, Codable{
    var id:  Int
    var avata: String
    var image: String
    var username: String
